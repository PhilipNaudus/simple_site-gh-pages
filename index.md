...
<script type="text/javascript">var placeholdersArr = [{{ placeholdersArr }}]; function imgPlace() { for(var i=0; i<{{imageCount}}; i++) { var pImg = document.getElementById('i'+i); pImg.style.backgroundSize = "cover"; pImg.style.backgroundImage="url(data:image/svg+xml;base64,"+placeholdersArr[i]+")"; pImg.style.width="100%"; pImg.height = (pImg.offsetWidth*0.6557);}} window.addEventListener("resize", imgPlace); imgPlace();
</script>
